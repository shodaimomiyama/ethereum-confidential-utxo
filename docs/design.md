...
金額の秘匿と使用権限の検証を区別するため、候補の暗号部品を次の意味で用いる。
...
鍵の具体的な生成と保存、公開受取情報の符号化、暗号化方式は別途具体化する。

認可対象には、入力、出力と所有者、金額への暗号的な結合、公開入出金、出金先、受領データ、対象環境、操作の再使用を識別する情報を含める。
署名形式は、secp256k1のECDSAと[EIP-712](https://eips.ethereum.org/EIPS/eip-712)による構造化データの署名を具体化案とする。
...
64 bitの範囲証明は、`v - 1` が `0` から `2^64 - 1` の範囲にあることを表す構成を候補とする。
金額 `v` そのものは上限で65 bitを必要とするため、`uint64` へ切り詰めない。
公開金額と通常の金額計算には、少なくともこの上限と入力合計を扱える整数型を使う。
受領データに `v` と `v - 1` のどちらを格納するかは正規符号化で固定し、復号時に区別する。
...
送金と出金で生成する出力については、`C - H` の開示値 `v - 1` に64 bitの範囲証明を適用する案を使う。
...
先に決まる対象チェーン、デプロイ、識別用値、出力の位置、所有者、コミットメントなどに受領暗号文を結び付け、暗号文を作成した後に操作IDを計算する構成を候補とする。
...
認可鍵から独立したX25519鍵を受領用鍵とする案を具体化する。
...
公開受取情報は、所有者、受領公開鍵、形式と方式の版、対象チェーンとデプロイを所有者署名で結ぶ案とする。
送信CLIは期待する所有者とその署名を照合する。
この署名だけでは、受取人が復号秘密鍵を保持していることや、現実の人物の身元まで証明しない。
正常な受取情報の生成時には、CLIが保持する受領秘密鍵と公開鍵の対応を確認する。
...
以下は合意した構成を実装へ対応付ける案である。
以下の構成要素は、初期版の論理ABIを固定してもなお、コンパイル可能なPool実装の完成とは区別する。
...
`kind` は入金0、送金1、出金2とし、それ以外を拒否する案とする。
...
可変長の入出力列を使い、物理的なダミーUTXOを導入しない案とする。
...
ECDSAの署名は65 byteの `r || s || v`、`v` は27または28とし、`r` の正規範囲とlow-sを本体で検査する案とする。
...
UTXO IDから所有者、コミットメント、状態を引くmappingを候補とする。
状態は不存在・未使用・消費済みを明示し、消費後も同じIDを再発行しない。
成功済み操作IDは別のmappingへ保持する。
暗号化受領packetはイベントに掲載し、ストレージへ同じ全データを重複保存しない。
保存量と同期時の履歴依存の双方を評価する。

公開入金累計から公開出金累計を差し引く帳簿残高を `accountedLiability` として保持する案とする。
...
2. 操作ID、出力ID、認可対象を組み立て、成功済み操作、入力の存在・未使用・所有者、出力IDの衝突を検査する。
...
資産入口は `deposit`、`transfer`、`withdraw` に分け、各入口に操作内容、認可署名、必要な金額証明を渡す案とする。
...
| 通知・照会の案 | 含める情報と用途 |
| --- | --- |
| `OperationSucceeded` | 操作ID、種別、認可所有者、入力ID列、出力ID列、公開入出金額、公開出金先、識別用乱数。操作の効果と受領contextを追跡する |
| `OutputCreated` | 索引付き所有者、UTXO ID、操作ID、出力位置、コミットメント、受領方式とpacket。発見・復号・照合に使う |
| `InputConsumed` | 入力UTXO ID、消費した操作ID。本人による使用と別意図の競合を照合する |
| `getUtxo` | 不存在・未使用・消費済み、所有者、コミットメント。照会したblockをCLIで記録する |
| `isOperationExecuted` | 操作IDの成功履歴。取引ハッシュとは別にコピー提出の成功を確認する |
| `getAccounting` | 実残高、帳簿残高、未計上ETH。個別の秘匿金額は返さない |

拒否理由は形式不正、範囲外、再入、入力不存在、重複、消費済み、所有者不一致、操作再使用、認可不成立、金額検証不成立、外部受領失敗を区別する案とする。
不正な金額の詳細を公開検証器から判別できない場合は、金額検証不成立へまとめる。
エラーに秘密入力を含めず、複数条件が不正な場合の優先順位を公開仕様として約束しない。
revertで消えるログを失敗記録として当てにしない。
...
| FR-06 | [受領保証](specification.md#受領の保証と不整合な受領データ) | 索引付き出力イベント | D-08 | 手動配送・専用サーバーなしの公開履歴取得 |
...
| SEC-03 | [再使用](specification.md#操作の再使用と競合) | mapping、昇順入力、実行時照合 | D-04とストレージ案 | 重複・不存在・消費済み・競合拒否、機械検証 |
...
| OPS-01 | [必須シナリオ](specification.md#必須の動作シナリオ) | ローカル実行と機械検証 | EXP-01の限定対象 | 全テスト・測定・必須命題をローカルまたは公開検証環境で再実行 |
...
| OPS-04 | [同期](specification.md#受領と同期の状態) | block hash照合、再取得、opId照合 | D-09、EIP-1898 | 通信失敗・未確定・再編成・コピー・競合をCLIで正しく識別 |
...
[EXP-11の事前計画](../experiments/design/formal/exp11/plan.json)と[記号claim](../experiments/design/formal/exp11/direct-symbolic-spec.k)では、8,467 byteの固定runtimeを初期`program`へ置き、ABIの公開座標語1つを`0 ≤ X < 2^256`の記号値とした。具体値`X=0`は既存の入力に対応し、前提は充足可能である。修正後のclaimは解析を通り、[2回目の実行結果](../experiments/design/formal/exp11/run-result-2.json)の保存グラフでは対象runtimeの実行へ進んだ。しかし約736秒後に`kore-rpc`との接続が切れ、Dockerの4 GiB VMで`oom_kill=1`を記録した。未解決nodeが2件あり、証明は完了していない。これは反例でも入力拒否の証明でもない。
