...
[PRD](PRD.md)、[要件定義](requirements.md)、[仕様書](specification.md)を満たす方式固有の詳細規則と、その採用根拠を定める。全体構成、実装基盤、モジュールとリポジトリの配置は[アーキテクチャ](architecture.md)を正本とする。
...
本書の役割は、仕様を満たす方式を選ぶため、その判断根拠と実装が従う詳細規則を示すことである。方式の妥当性を左右する問いは、一次資料、数式、既存実装、必要な最小実験で確認する。独立して進められるPoC実装は本草案を基準に別Issueで並行できるが、v3検証器やPoolとの結合は変更可能性を残し、未証明の経路を完成・要件達成として扱わない。全必須命題の形式証明、正式な性能比較、公開テストネットでの一連の検証は後続Issueの受入条件とする。
...
範囲証明には後述のv3を研究PoCの条件付き基準として選ぶ。本体との公開入力の結合と操作の符号化は設計規則として後述する。実装基盤は[アーキテクチャの方針](architecture.md#構成方針と採用状態)に従い、コントラクトはSolidity 0.8系へ移植し、共通クライアント処理はTypeScriptとする。依存の固定版と形式証明の第一経路は決定済みだが、結合を含む移植後の実コードでの成立と証明完了は未確認である。
...
| 形式検証の経路 | EXP-05で局所命題は成立。EXP-10の直接claimの構文問題はEXP-11で修正し、旧Solidity 0.4.19版v3 runtimeへの到達を確認した。EXP-11はメモリ不足、EXP-12は900秒の上限で未完了。EXP-13の高速バックエンドも証明完了を記録できなかった。v3実コードの命題は未証明 | [形式検証経路の採用判断](#v3実コードの形式検証経路)に従い、対象bytecodeで完了する代表命題を示すか、検証手法・実装構成を見直す |
...
選定した言語系列を無条件の採用根拠にせず、本体と検証器を機械検証できる実装構成・依存版の組合せとして評価する。
...
構成要素の責務、接続関係と配置は[システム全体構成](architecture.md#システム全体構成)を参照する。
...
本体と検証器の実コードへの形式検証は、Solidityから生成したEVMバイトコードをKontrol/KEVMで扱い、共通状態モデルと本体固有の検証関係をKで記述する構成を第一の証明経路として採用する。モデルの安全性と正常操作成立、検証器コードと検証規則の対応、Poolの認可・公開入力・呼出し境界・状態更新、検証関係からモデル条件への対応を別の命題として証明し、要件と対象artifactへ対応付ける。経路の選定は証明完了を意味せず、[移植後実コードの代表命題の完了条件](#v3実コードの形式検証経路)とFV-01〜FV-04の受入条件は維持する。
...
実験と正式実装の技術・依存版の区別、Kontrol/KEVMの実験固定版、EVM scheduleとcompilerの対応は[実験固定版と互換性](architecture.md#実験固定版と互換性)を参照する。0.8系への移植後も、意味論が必要な境界を表すことと、その実コードの命題を証明できることを区別する。
...
初回実装はアーキテクチャで固定したTypeScriptのhpke-js系列を使い、固定suiteのRFCテストベクトル、無効な鍵と改ざんデータの拒否、送受信プロセス停止後の受領と再使用を確認する。
...
全体構成と責務の正本は[アーキテクチャ](architecture.md#システム全体構成)とする。Poolとは別の固定金額検証器へ `STATICCALL` する方針を採る。以下の詳細規則で、Poolが実際の操作から公開入力を計算し、正しい検証器への呼出しと結果を使って受理・拒否する対応を定める。

範囲と収支を一つの金額検証器へ配置し、既存v3範囲ABIと収支検証の入口を分ける。具体的な境界は[操作の結合とABI](#操作の結合とabi)で固定し、移植後の接続試験と実コード証明は[U-01](architecture.md#決定事項の検証と見直し条件)で追跡する。任意の外部コントラクトの成功フラグを受理したり、検証器が正しいという仮定だけでFV-03を省略したりしない。
...
すべての出力の所有者はゼロアドレス以外とし、ゼロアドレスの出力所有者を指定した操作は拒否する。
出金先もゼロアドレス以外とし、ゼロアドレスへの出金を拒否する。Pool自身を出金先に指定することは許す。
...
一つの金額検証器の公開入口を次の宣言へ固定する。範囲検証の関数名と引数の型・順序を維持し、収支検証を別のselectorへ分離する。いずれも資産状態を更新しない。
...
Poolは両入口を固定先へ直接`STATICCALL`する。収支検証器は上記challengeの`chainId`を`block.chainid`、`pool`を現在の呼出し元`msg.sender`から得る。したがってPool経由の検証ではPoolのアドレスに結び付き、検証器自身の`address(this)`や取引の`tx.origin`を使わない。これらの[変数の意味](https://docs.soliditylang.org/en/latest/units-and-global-variables.html#block-and-transaction-properties)に従い、間に別の呼出しコントラクトを挟まない。これは所有者認可ではなく収支証明の文脈の結合であり、所有者は引き続きPoolの署名検証で決める。CLIが検証器を直接`eth_call`する場合も対象チェーンと`from = Pool`を指定し、その結果を操作全体の受理・確定とは扱わない。

`verifyBalance`はPool計算の`operationId`と`X`を受け取り、正規な`X`・非単位元の`R`・`s`、challenge抽出、`sG = R + cX`を検査する。正規な`X = (0,0)`は許可する。`verify`は既存v3のtranscript・点・scalarの規則と`ls.length = rs.length = 12`を維持する。両入口とも検証条件の不成立を成功として返さず、呼出し失敗もPoolが拒否する。

Poolは呼出しの成功フラグが真で、返値がちょうど32 byteかつその符号なし整数値が1の場合だけ、その証明を受理する。false、revert、空・短すぎる・長すぎる返値、0と1以外の値は拒否する。Poolが処理可能な失敗は[拒否理由のABI](#拒否理由のabi)の対応する証明エラーへ正規化する。この境界の試験には、呼出し元・チェーン・操作ID・公開点の改変、単位元の正当例、異常な返値とprecompile失敗を含める。境界の固定は、移植後の接続試験や実コード証明の完了を意味しない。
...
| エラー宣言 | 対応する拒否条件 |
| --- | --- |
| `InvalidRequest()` | 操作種別・個数・出力所有者の役割・入力順序・未使用フィールド・受領形式やpacket長が不正。出力所有者または出金先がゼロアドレス。出力コミットメントが非正規または曲線外。証明件数が不一致 |
...
通常取引による提出、作成・提出・同期の責務と秘密情報の境界は[主要なデータフロー](architecture.md#主要なデータフロー)、CLIの配置は[モジュール構成](architecture.md#モジュール構成)を参照する。中継経路でも所有者は検証した署名から決まり、`msg.sender`や`tx.origin`から決めない。入金とgasの資金源、認可の対象、再提出時の判定は本書の詳細規則を維持する。

### CLIの暗号化保存規則

対応要件: SEC-06、OPS-04。保存責務とバックアップ範囲は[CLIの永続化と復旧](architecture.md#cliの永続化と復旧)に従う。以下はローカル保存profile v1であり、オンチェーンABIやHPKEの受領packet形式とは別に版を管理する。

#### 保存profile v1

| 項目 | 固定規則 |
| --- | --- |
| 鍵導出 | Node組込みの非同期scrypt、`N=131072, r=8, p=1`、出力32 byte、`maxmem=268435456` byte |
| 暗号 | AES-256-GCM、nonce 12 byte、認証tag 16 byte。復号時にもtag長を明示する |
| 乱数 | 保存・バックアップの再暗号化ごとにCSPRNGからsalt 16 byteとnonce 12 byteを新たに採取する |
| 外側の形式 | UTF-8 JSON object。フィールドは文字列の `header`、`ciphertext`、`tag` の3つだけとし、各値は標準alphabet・padding付きの正規base64でbyte列を表す |
| header | 復号したUTF-8 JSON objectのフィールドは `version:1`、`profile:"scrypt-aes256gcm-v1"`、base64文字列の `salt` と `nonce`。保存されたheaderのbyte列そのものをAADとし、再serializeしない |
| 入力検査 | 外側・headerとも重複キー、未知フィールド、不正な型・UTF-8・base64・長さ、未知版・profileを拒否する。暗号化ファイル全体は16 MiB以下とし、KDF開始前に検査する |
| 復号 | GCMの認証が成功するまで、復号途中の平文をparse・利用・出力しない。失敗時は保存状態を更新しない |

scryptのパラメータは本プロジェクトの固定値であり、RFCの必須推奨値を意味しない。作業領域は概算128 MiBとなるため、Nodeの既定`maxmem`を使わない。外部ファイルから任意のKDFパラメータを受け取らず、上記profileだけを実行する。根拠は[Node 24.21.0の暗号API](https://github.com/nodejs/node/blob/v24.21.0/doc/api/crypto.md)と[RFC 7914](https://www.rfc-editor.org/rfc/rfc7914.html)とする。

暗号化する内容には、受領秘密鍵と対応する公開鍵、チェーン・Pool・所有者の識別、秘密を含む操作作成データ、提出の追跡情報を含める。同一操作の再生成に必要な情報は[中断と再生成の規則](#証明生成の中断と再生成)に従って保持する。復号済みUTXOや同期cursorをキャッシュする場合も暗号化領域に置く。内側のデータはschemaの版を保持し、未知schemaを黙って読み替えない。公開提出用ファイルには秘密を含めず、保存上限に達した場合も未完了操作や鍵を自動削除しない。

パスフレーズは非表示のTTY入力で受け取り、初回と変更時は再入力で照合する。UTF-8として扱い、trimや暗黙のUnicode正規化をしない。コマンドライン引数・環境変数・通常ログへ渡さない。解除中に必要な秘密だけを保持し、終了・失敗時に所有するBufferを可能な範囲で消去する。JavaScript文字列、GC、OSのswapまで完全に消去する保証は置かない。

#### 更新と復旧

ローカル保存先は利用者専用ディレクトリ（0700）、ファイル（0600）とし、所有者・権限・symlinkを検査する。保存先ごとに単一writerを排他的に確保する。更新に使う最新状態を読み出す前にロックを取得し、復号・検査・変更・暗号化・置換から親ディレクトリの同期完了まで保持する。ロックを取得できない場合は更新を開始しない。ロック取得前に読んだスナップショットを、そのまま更新元として書き戻してはならない。同期や証明生成などをロック外で行った場合も、ロック取得後に最新状態を読み直し、変更の適用条件を再検査する。他の処理が保存した鍵・操作を含む状態全体を古いスナップショットで置換しない。残留ロックをPIDだけで安全と判断して自動削除しない。

更新は同じディレクトリ内に排他的に作成した一時ファイルへ暗号文を書き、ファイルの`fsync`、`rename`、親ディレクトリの`fsync`の順で行ってから成功を返す。平文の一時ファイルは作らない。中断時は置換前か置換後の完全な状態を使い、残留一時ファイルを自動で最新版と認定しない。永続化の成否が不明な場合は成功と報告せず、作成した操作の提出へ進まない。原子的な置換と電源断後の永続性は別に検証する。[Nodeのファイル同期](https://nodejs.org/docs/latest-v24.x/api/fs.html#filehandlesync)、[fsyncの意味](https://man7.org/linux/man-pages/man2/fsync.2.html)

バックアップは一貫した保存状態を上記形式で再暗号化し、作成時点を暗号化領域へ記録する。復元は既存状態を黙って上書きせず、別の保存先で認証・schema・鍵対応・操作IDを検査する。その後にチェーンから再同期し、[再編成と再提出](#再編成と再提出)に従って成功履歴・入力状態・認可状態を照合する。自動再提出はしない。

パスフレーズ忘れに対する復号・復旧機能は持たない。変更後も古いバックアップは旧パスフレーズで復号できる。正しく認証される古いバックアップへの差し戻しはAEADだけでは検出できず、ローカルの世代番号だけでも防げない。取得後に作った鍵や未提出操作の回復、所有者の署名鍵の復旧まで保証しない。

#### 採用理由と検証

scryptとAES-GCMはNode標準APIで実装し、初期CLIの方式を一つに固定する。Argon2idも採用Nodeの組込みAPIで利用できる代替だが、scrypt APIの利用実績と固定profileの単純さを優先する。OSキーストアはOSごとの実装・バックアップ方法が増え、メモリ保持だけでは別の鍵保管手段を必要とするため、初期CLIの永続化方式には選ばない。

受入試験ではRFCベクトル、改ざん・誤パスフレーズ・未知版・過大入力の拒否、権限と同時writerの拒否、各更新段階での中断、保存失敗時の提出停止、別プロセスでのバックアップ復元と再同期を確認する。並行する二つのCLIが同じ旧状態を参照し、一方が新しい受領鍵・未完了操作を保存した後に、もう一方が同期結果を保存する試験も行う。後の更新が最新状態を読み直し、先に保存した鍵・操作を保持することを確認する。旧バックアップからの復元で残高を二重計上したり成功済み操作を自動提出したりしないことも検査する。これらは未実施であり、文書上の採用を検証済みと扱わない。
...
対応要件: OPS-01〜OPS-05、EVAL-04。対応仕様: [設計で具体化する事項](specification.md#設計で具体化する事項)。

言語と技術の採用状態、実験版との区別は[採用技術と代替案](architecture.md#採用技術と代替案)、ローカル・公開環境と再現条件は[開発・検証・実行環境](architecture.md#開発検証実行環境)、配置と各担当への依存は[後続作業への引継ぎ](architecture.md#後続作業への引継ぎ)を正本とする。

本書は方式と詳細規則への適合条件と実験の証拠を保持する。独立した実装は暫定基準で並行できるが、未決の方式・結合・証明経路を確定済みとして扱わない。移植後の成立性確認と、全必須命題の証明・正式評価・公開テストネットの受入検証を区別する。
...
現時点で、EXP-05の小さい群演算ライブラリの命題を、最適化済みv3検証器の形式検証可能性へ一般化する根拠はない。上記EXP-09〜13はSolidity 0.4.19版の固定runtimeに対する履歴として保持する。

[Solidity 0.8系への移植方針](architecture.md#構成方針と採用状態)に伴い、今後の採用条件の対象を移植後の実コードへ切り替える。**詳細設計確定前の条件として、移植後のv3検証器の固定bytecodeで代表的な入力拒否またはprecompile境界の命題を、対象コードへの到達と非空虚な証明完了を含めて示す。** compilerの完全版、optimizer・viaIR、EVM target、ソースとruntimeのhashを固定し、旧profileの規則に対応する相互運用・正規化・単位元・演算境界を再確認する。Poolからの `STATICCALL` とその引数・返値・失敗の利用も実コードの証明経路へ対応付ける。

これは対象コードの選び直しであり、採用条件やFV要件の緩和ではない。旧コードのテスト、gas、局所命題を移植後の証拠へ流用しない。Kontrol/KEVM候補で成立しない場合は、対象命題の分解、証明ツール、または検証器の実装構成を比較し、FV-03〜FV-04の全命題を後続PoCで完了できる経路を選び直す。単なるタイムアウト延長、具体テスト、harnessだけの証明を採用判断の代用にしない。全命題の実証、第三者による再実行、正式な資源・gas評価は引き続きPoCの受入条件である。
