...
export const scenarios: readonly Scenario[] = [
  ...
  scenario('S-25/other-attempt-pending', [event({ type: 'attempt-failed', card: 'pay', operationId, otherAttemptPending: true, inputUnspent: true, deadlineValid: true }), action({ type: 'retry-attempt', operationId }, 'blocked')], { card: 'pay', phase: 'failed', chainOutcome: 'finalized-failure', allowedActions: ['recheck'], forbiddenActions: ['retry-attempt', 'start:pay'] }),
  ...
  scenario('S-45/distribution-attempt-failed', [event({ type: 'attempt-failed', card: 'reward', operationId, otherAttemptPending: true, inputUnspent: true, deadlineValid: true })], { card: 'reward', phase: 'failed', chainOutcome: 'finalized-failure', allowedActions: ['recheck'], forbiddenActions: ['start:reward', 'retry-attempt'] }),
