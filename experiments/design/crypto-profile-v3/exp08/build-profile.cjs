...
const vectors = JSON.parse(fs.readFileSync(path.join(root, '../../crypto-profile-v2/vectors.json')));
...
const tags = Object.fromEntries(Object.entries(baseline.tags).map(([name, label]) => [name, label.replaceAll('/v1', '/v3')]));
...
const profile = { ...baseline, experiment: 'EXP-08', status: 'Design-stage v3 candidate; not security reviewed', tags,
  ...
  challenge: 'segment=tag(stage)||uint256(payload byte length)||payload; first c=uint256(K(prefix||segment||tag(candidate)||uint256(counter))) with 0<c<q and counter in [0,255] is accepted; prefix=prefix||segment||tag(accepted)||uint256(c); no modular reduction',
  transcript: 'Direct full-prefix; each challenge hashes an explicitly encoded concatenation of all earlier stage data. The trace state is only K(prefix), never substituted for the prefix in challenge derivation. No intermediate hash of prefix||segment is used.',
...
profile.pointRules = 'C_range and proof points A,S,T1,T2,L,R may use canonical identity (0,0); fixed generators must be nonidentity. Every nonidentity point has x,y<p and y^2=x^3+3 mod p.';
profile.randomness = 'SecureRandom rejection sampling in [0,q-1], including zero; no retry solely for an internal identity point. Abort without a partial proof if a 256-candidate scalar or challenge search exhausts.';
profile.proverLimits = { proofAttempts: 1, secretScalarCandidatesPerSample: 256 };
