...
const { ContractFactory, JsonRpcProvider, keccak256, toUtf8Bytes } = load('ethers');
...
async function main() {
  ...
  const result = { experiment: 'EXP-07B', compiler: solc.version(), optimizerRuns: 200,
  ...
  const anvil = spawn('anvil', ['--host', '127.0.0.1', '--port', '18549', '--hardfork', 'prague', '--chain-id', '31337', '--gas-limit', '30000000', '--quiet'], { stdio: ['ignore', log, log] });
  const provider = new JsonRpcProvider('http://127.0.0.1:18549', 31337, { staticNetwork: true });
  try {
    let ready = false;
    for (let i = 0; i < 50; i++) {
