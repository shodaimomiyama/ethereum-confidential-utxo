...
contract RangeProofVerifier {
    ...
    bytes32 internal constant PARAMETERS_TAG = keccak256("ecu/bp/parameters/v1");
    bytes32 internal constant Y_TAG = keccak256("ecu/bp/y/v1");
    bytes32 internal constant Z_TAG = keccak256("ecu/bp/z/v1");
    bytes32 internal constant X_TAG = keccak256("ecu/bp/x/v1");
    bytes32 internal constant U_TAG = keccak256("ecu/bp/u/v1");
    bytes32 internal constant ROUND_TAG = keccak256("ecu/bp/round/v1");
    ...
    function RangeProofVerifier(uint256[4] base, uint256[128] gsCoords, uint256[128] hsCoords) public {
        ...
        // whose 130 distinct points are checked off chain (EXP-03 parameters.json).
    ...
    function verify(bytes32 operationId, uint256 outputIndex, uint256[10] coords, uint256[5] scalars, uint256[] ls, uint256[] rs) external view returns (bool) {
        ...
        return verifyRange(inputCommitment, proof, Transcript.initialState(operationId, outputIndex, inputCommitment));
    }
    ...
    function decodeProof(uint256[10] coords, uint256[5] scalars, uint256[] ls, uint256[] rs) internal pure returns (RangeProof proof) {
        ...
        require(alt_bn128.isAllowedPoint(proof.commitmentA, false));
        require(alt_bn128.isAllowedPoint(proof.commitmentS, false));
        require(alt_bn128.isAllowedPoint(proof.coefficientCommitments[0], false));
        require(alt_bn128.isAllowedPoint(proof.coefficientCommitments[1], false));
        ...
        for (index = 0; index < ROUNDS; index++) {
            ...
            require(alt_bn128.isAllowedPoint(proof.left[index], false));
            require(alt_bn128.isAllowedPoint(proof.right[index], false));
        }
    }

    function verifyRange(alt_bn128.G1Point inputCommitment, RangeProof proof, bytes32 initialState) internal view returns (bool) {
    ...
    function verifyInner(bytes32 state, alt_bn128.G1Point commitment, alt_bn128.G1Point innerBase, alt_bn128.G1Point[BIT_WIDTH] hPrimes, RangeProof proof) internal view returns (bool) {
    ...
    function challengeForPoints(bytes32 state, bytes32 stageTag, alt_bn128.G1Point first, alt_bn128.G1Point second) internal pure returns (bytes32 nextState, uint256 challenge) {
    ...
    function challengeForScalars(bytes32 state, uint256 tauX, uint256 mu, uint256 t) internal pure returns (bytes32 nextState, uint256 challenge) {
    ...
    function challengeForRound(bytes32 state, uint256 roundIndex, alt_bn128.G1Point left, alt_bn128.G1Point right) internal pure returns (bytes32 nextState, uint256 challenge) {
