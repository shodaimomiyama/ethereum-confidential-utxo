...
def prove_in_container():
    ...
    plan = json.loads((evidence / "exp05-revised-sub-proof-plan.json").read_text())
...
def prove_from_host():
    output_path = DIRECTORY / "exp05-revised-sub-claim.json"
    if output_path.exists():
        raise SystemExit("Refusing to overwrite: " + str(output_path))
    command = [
        "docker", "run", "--rm", "--platform", "linux/amd64",
        "--name", "exp05-revised-sub-claim", "--cpus", "4", "--memory", "8g",
        ...
        "--workdir", "/tmp", IMAGE, "python3", "-B", "/evidence/prove_exp05_revised_sub.py", "container",
    ]
    record = {
