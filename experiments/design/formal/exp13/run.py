...
def inside():
    ...
    plan = json.loads((evidence / "exp13/plan.json").read_text())
    ...
    project = pathlib.Path("/tmp/exp13")
    project.mkdir()
    ...
    shutil.copytree(evidence / "out/kompiled", definition)
    ...
    if record["serialization"]["exitCode"] == 0:
        record["proof"] = execute([
            ...
            "--save-directory", str(project / "proof"), "--use-booster",
...
def outside():
    if (EXPERIMENT / "run-result.json").exists():
        raise SystemExit("Refusing to overwrite EXP-13 result")
    command = [
        "docker", "run", "--rm", "--platform", "linux/amd64", "--name", "exp13-direct-v3-proof",
        "--cpus", "4", "--memory", "4g", "--memory-swap", "4g", "--network", "none",
        ...
        IMAGE, "python3", "-B", "/evidence/exp13/run.py", "inside",
    ]
    result = execute(command, 740)
