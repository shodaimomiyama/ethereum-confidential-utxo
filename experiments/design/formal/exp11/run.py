...
def inside():
    ...
    plan = json.loads((evidence / "exp11/plan.json").read_text())
    ...
    project = pathlib.Path("/tmp/exp11")
    project.mkdir()
    ...
    proof = project / "proof"
    if proof.exists():
        destination = output / "proof-snapshot-2"
    ...
    (output / "run-result-2.json").write_text(json.dumps(record, indent=2) + "\n")
    print(json.dumps({"result": str(output / "run-result-2.json"), "proof": record.get("proof", {}).get("exitCode"), "timedOut": record.get("proof", {}).get("timedOut")}))


def outside():
    if (EXPERIMENT / "run-result-2.json").exists():
        raise SystemExit("Refusing to overwrite EXP-11 second result")
    command = [
        "docker", "run", "--rm", "--platform", "linux/amd64", "--name", "exp11-direct-v3-proof",
        ...
        IMAGE, "python3", "-B", "/evidence/exp11/run.py", "inside",
    ]
